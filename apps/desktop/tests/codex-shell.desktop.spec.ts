...
async function appPage(browser: Browser): Promise<Page> {
  ...
  await page.evaluate(() => { localStorage.setItem("mobius.onboarding.complete", "1"); localStorage.setItem("mobius.page", "sessions"); });
  await page.reload();
  return page;
}
...
test("project/session shell and keyboard search use one navigation surface", async () => {
  ...
  try {
    ...
    await expect(page.locator(".project-sidebar-nav .rail-item")).toHaveCount(3);
