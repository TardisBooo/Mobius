...
import { Check, ChevronDown, FolderGit2, Languages, LoaderCircle, Maximize2, Minus, Moon, PanelLeft, PanelRight, Plus, RefreshCw, Search, Sparkles, Sun, TerminalSquare, X } from "lucide-react";
...
const WorkspaceInspector = lazy(() => import("./WorkspaceAtlas").then((module) => ({ default: module.WorkspaceInspector })));
...
function language(locale: "zh-CN" | "en") {
  ...
  return {
    workbench: zh ? "工作区" : "Workspaces", sessions: "Agents", notes: zh ? "资料" : "Library", canvas: zh ? "画布" : "Canvas", documents: zh ? "资料库" : "Documents", skills: zh ? "技能" : "Skills", settings: zh ? "设置" : "Settings",
    search: zh ? "搜索 Agent、会话、资料和设置" : "Search agents, sessions, library and settings", scan: zh ? "扫描会话" : "Scan sessions",
...
export function App() {
  ...
  const [page, setPage] = useState<Page>(() => {
    ...
    return saved === "workbench" || saved === "notes" || saved === "skills" || saved === "settings" ? saved : "sessions";
  });
  ...
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(() => localStorage.getItem("mobius.project.selected"));
  const [sidebarOpen, setSidebarOpen] = useState(() => localStorage.getItem("mobius.sidebar.open") !== "0");
  const [sessionToolRequest, setSessionToolRequest] = useState<"sources" | "memory" | null>(null);
  const [libraryMode, setLibraryMode] = useState<"documents" | "canvas">("documents");
  const [documentsVisited, setDocumentsVisited] = useState(page === "notes");
  ...
  const [attachedSessions, setAttachedSessions] = useState<Record<string, string>>({});
  ...
  useEffect(() => { localStorage.setItem("mobius.sidebar.open", sidebarOpen ? "1" : "0"); }, [sidebarOpen]);
  useEffect(() => { if (selectedProjectId) localStorage.setItem("mobius.project.selected", selectedProjectId); }, [selectedProjectId]);
  useEffect(() => {
    if (page !== "notes") return;
    if (libraryMode === "documents") setDocumentsVisited(true);
    else setCanvasVisited(true);
  }, [page, libraryMode]);
  ...
  useEffect(() => {
    ...
    let active = true;
    void desktopApi.listTerminals().then((terminals) => {
      if (!active) return;
      const live = new Set(terminals.map((terminal) => terminal.id));
      setAttachedSessions((current) => Object.fromEntries(Object.entries(current).filter(([, terminalId]) => live.has(terminalId))));
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k" && !(event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement)) { event.preventDefault(); setSidebarOpen(true); setPage("sessions"); window.setTimeout(() => window.dispatchEvent(new Event("mobius:focus-search")), 0); }
    };
  ...
  const createFreeTerminal = useCallback(async () => {
    try {
      ...
      const selectedId = selectedProjectId ?? localStorage.getItem("mobius.workspace.current");
      const selected = current.find((workspace) => workspace.workspace.id === selectedId) ?? current[0];
      const suggested = selected?.checkouts[0]?.canonical_path ?? await desktopApi.pickDirectory();
      if (!suggested) return;
      openTerminal(await desktopApi.createTerminal(suggested, "PowerShell"));
    } catch (reason) { setError(String(reason)); }
  }, [openTerminal, selectedProjectId]);
  ...
  const selectedProject = workspaces.find((item) => item.workspace.id === selectedProjectId) ?? null;
  const title = page === "notes" ? text.notes : page === "settings" ? text.settings : page === "workbench" ? (workbenchPanel === "terminal" ? text.workbench : selectedProject?.workspace.display_name ?? text.workbench) : page === "sessions" ? text.sessions : text.skills;
  const openProject = (id: string) => { setSelectedProjectId(id); setWorkbenchPanel("workspaces"); setPage("workbench"); };
  const addProject = async () => { try { const path = await desktopApi.pickDirectory(); if (!path) return; const inspection = await desktopApi.registerWorkspace(path); await reload(); if (inspection) openProject(inspection.workspace.id); } catch (reason) { setError(String(reason)); } };

  return <div className={`mobius-app codex-shell ${sidebarOpen ? "" : "sidebar-collapsed"} ${terminalFocus ? "terminal-focus" : ""}`}>
    <button className="sidebar-toggle" type="button" aria-label={sidebarOpen ? (locale === "zh-CN" ? "收起侧栏" : "Collapse sidebar") : (locale === "zh-CN" ? "展开侧栏" : "Expand sidebar")} title={sidebarOpen ? (locale === "zh-CN" ? "收起侧栏" : "Collapse sidebar") : (locale === "zh-CN" ? "展开侧栏" : "Expand sidebar")} aria-expanded={sidebarOpen} onClick={() => setSidebarOpen((value) => !value)}><PanelLeft size={17}/></button>
    <header className="mobius-topbar" data-tauri-drag-region>
      <div className="mobius-brand" data-tauri-drag-region><MobiusLogo size={25}/><strong>MÖBIUS</strong></div>
      <button className="global-search" type="button" onClick={() => { setSidebarOpen(true); setPage("sessions"); window.setTimeout(() => window.dispatchEvent(new Event("mobius:focus-search")), 0); }}><Search size={17}/><span>{text.search}</span><kbd>Ctrl K</kbd></button>
      <div className="topbar-actions"><button className="top-icon" type="button" onClick={() => setOnboarding(true)} aria-label="Open Möbius guide"><Sparkles size={17}/></button><button className="top-icon" type="button" onClick={() => setLocale(locale === "zh-CN" ? "en" : "zh-CN")} aria-label="Switch language"><Languages size={17}/><span>{locale === "zh-CN" ? "EN" : "中文"}</span></button><button className="top-icon" type="button" onClick={() => setTheme(theme === "light" ? "dark" : "light")} aria-label="Switch theme">{theme === "light" ? <Moon size={17}/> : <Sun size={17}/>}</button><span className="window-controls"><button type="button" onClick={() => void windowAction("minimize")} aria-label="Minimize"><Minus size={15}/></button><button type="button" onClick={() => void windowAction("maximize")} aria-label="Maximize"><Maximize2 size={14}/></button><button className="window-close" type="button" onClick={() => void windowAction("close")} aria-label="Close"><X size={15}/></button></span></div>
    </header>
    <ProjectSidebar workspaces={workspaces} page={page} locale={locale} revision={sessionRevision} refreshing={scanning} onNavigate={(target) => { setTerminalFocus(false); setPage(target); }} onOpenProject={openProject} onAddProject={() => void addProject()} onOpenSession={(focus) => { setSessionFocus(focus); setPage("sessions"); }} onOpenSessionTool={(tool) => { setSessionToolRequest(tool); setPage("sessions"); }} onRefresh={() => void refresh()} onRenamed={() => setSessionRevision((value) => value + 1)} onError={setError}/>
    <main className={`mobius-main page-${page}`}><header className={`pagebar pagebar-${page}`}><div><span>{page === "workbench" ? workbenchPanel === "terminal" ? "WORKSPACE / POWERSHELL" : "PROJECTS / WORKSPACES" : page === "notes" ? "WORKSPACE / LIBRARY" : page === "settings" ? "APP / SETTINGS" : "AGENTS / RECENT ACTIVITY"}</span><h1>{title}</h1></div>{page === "workbench" ? <div className="pagebar-actions">{workbenchPanel === "terminal" ? <><button className="soft-button" type="button" onClick={() => { setTerminalFocus(false); setWorkbenchPanel("workspaces"); }}><FolderGit2 size={16}/>{text.showWorkspaces}</button><button className="soft-button" type="button" onClick={() => setTerminalFocus((value) => !value)}>{terminalFocus ? <ChevronDown size={16}/> : <PanelRight size={16}/>} {terminalFocus ? text.exitFocus : text.focus}</button></> : <><button className="soft-button" type="button" onClick={() => setWorkbenchPanel("terminal")}><TerminalSquare size={16}/>{locale === "zh-CN" ? `终端 ${terminalCount}` : `Terminals ${terminalCount}`}</button><button className="primary-button" type="button" onClick={() => void createFreeTerminal()}><Plus size={16}/>{locale === "zh-CN" ? "新建 PowerShell" : "New PowerShell"}</button></>}</div> : page === "sessions" ? <button className="primary-button" type="button" onClick={() => void refresh()} disabled={scanning}>{scanning ? <LoaderCircle className="spin" size={16}/> : <RefreshCw size={16}/>} {text.scan}</button> : page === "skills" ? <button className="soft-button" type="button" onClick={() => setPage("workbench")}><TerminalSquare size={16}/>{text.workbench}</button> : null}</header><section className="mobius-page-host">
      {page === "workbench" && workbenchPanel === "workspaces" ? selectedProject ? <section className="project-context-view"><Suspense fallback={<Loading label={text.loading}/>}><WorkspaceInspector item={selectedProject} initialCheckoutId={null} openTerminal={openTerminal} onError={setError} locale={locale} onOpenSessions={(focus) => { setSessionFocus(focus); setPage("sessions"); }}/></Suspense></section> : <div className="mobius-empty"><FolderGit2 size={28}/><p>{locale === "zh-CN" ? "从左侧选择项目，或添加项目" : "Choose a project from the sidebar, or add one"}</p><button className="primary-button" onClick={() => void addProject()}>{locale === "zh-CN" ? "添加项目" : "Add project"}</button></div> : null}
      ...
      {page === "sessions" ? <Suspense fallback={<Loading label={text.loading}/>}><SessionLibraryV2 revision={sessionRevision} indexing={indexing} workspaces={workspaces} health={health} attachedSessionIds={Object.keys(attachedSessions)} openTerminal={(terminal, sessionId) => { if (sessionId) setAttachedSessions((current) => ({ ...current, [sessionId]: terminal.id })); openTerminal(terminal); }} onError={setError} onToast={setToast} locale={locale} focus={sessionFocus} onFocusConsumed={() => setSessionFocus(null)} toolRequest={sessionToolRequest} onToolRequestConsumed={() => setSessionToolRequest(null)} embedded/></Suspense> : null}
      {documentsVisited ? <div className="library-route-surface" hidden={page !== "notes" || libraryMode !== "documents"}><Suspense fallback={<Loading label={text.loading}/>}><NotesLibraryV2 onError={setError} onToast={setToast} locale={locale} onOpenCanvas={(boardId) => { setRequestedBoardId(boardId); setLibraryMode("canvas"); }}/></Suspense></div> : null}
      {canvasVisited ? <div className="library-route-surface" hidden={page !== "notes" || libraryMode !== "canvas"}><Suspense fallback={<Loading label={text.loading}/>}><BoardPage onToast={setToast} initialBoardId={requestedBoardId} onBackToLibrary={() => { setRequestedBoardId(undefined); setLibraryMode("documents"); }}/></Suspense></div> : null}
      ...
      {page === "settings" ? <Suspense fallback={<Loading label={text.loading}/>}><SettingsPage locale={locale} theme={theme} onTheme={setTheme} onLocale={setLocale} onError={setError} onToast={setToast}/></Suspense> : null}
    </section></main>
