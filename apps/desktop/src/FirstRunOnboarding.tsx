...
function copy(locale: "zh-CN" | "en") {
  ...
  return {
    ...
    valueBody: zh ? "莫比乌斯把散落在 Codex、Claude Code 与 Pi 中的会话重新组织为 Agent 工作流。找回原会话、在真实 PowerShell 中继续，或把可追溯的 Session 来路交给另一个 Agent。" : "Möbius organizes sessions from Codex, Claude Code, and Pi as one agent workflow. Find the original session, resume it in real PowerShell, or hand another agent a traceable path to the source sessions.",
    ...
    continueBody: zh ? "原生 resume 保留原 Agent；跨 Agent 交接只传递 Session 引用图谱，由目标按需读取原始记录。" : "Native resume keeps the original agent; cross-agent handoff carries Session references so the target can read original records as needed.",
    ...
    noAutoBody: zh ? "主动复制 @session、运行检索或确认交接也不会复制完整历史；目标 Agent 决定是否通过受限读取接口查看原文。" : "Copying an @session reference, searching, or approving a handoff still does not copy full history. The target decides whether to read source ranges through the bounded interface.",
    ...
    sourceBody: zh ? "莫比乌斯探测常规 Harness 根目录。你可以在 Agents 页添加或移除明确来源；缺少适配的目录不会被全盘扫描。" : "Möbius probes conventional harness roots. Add or remove explicit sources from Agents; unsupported locations are never discovered by scanning an entire disk.",
    ...
    sourceEmpty: zh ? "当前未发现来源。稍后可在“Agents → 来源”中选择目录。" : "No source found yet. Choose a folder later from Agents → Sources.",
    ...
    handoffTitle: zh ? "换 Agent 时，传递可追溯来路，不复制一份摘要。" : "When changing agents, pass a traceable path instead of copying a summary.",
    handoffBody: zh ? "选择“交接给其他 Agent”，确认源 Session、祖先图谱、目标 Harness 和 Workspace。目标原生 Session 身份核验后才会建立交接边；原始记录始终只读。" : "Choose Handoff to another agent and review the source Session, ancestor graph, target Harness, and Workspace. A lineage edge is created only after the target native Session identity is verified; source records remain read-only.",
    exactTitle: zh ? "知道来源：精确引用" : "Know the source: exact reference",
    exactBody: zh ? "复制 @session:provider/id#mN，粘贴到任何 Agent；引用不包含原文副本。" : "Copy @session:provider/id#mN into any agent. The reference contains no transcript copy.",
  ...
  return <div className="onboarding-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <section ref={dialogRef} className="onboarding" role="dialog" aria-modal="true" aria-labelledby="onboarding-title" aria-describedby="onboarding-summary">
      ...
      <main className="onboarding-content">
        ...
        {step === 4 ? <StepPage signal="SESSION LINEAGE" title={text.handoffTitle} body={text.handoffBody}><Evidence icon={<AtSign/>} title={text.exactTitle} body={text.exactBody}/><Evidence icon={<GitFork/>} title={text.recallTitle} body={text.recallBody}/></StepPage> : null}
