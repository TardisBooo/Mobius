...
export function WorkspaceAtlas({ workspaces, reload, openTerminal, onError, onToast, onOpenSessions, locale }: {
  ...
  const beginDrag = (event: DragEvent<HTMLElement>, id: string) => { event.dataTransfer.setData("application/x-mobius-workspace", id); event.dataTransfer.effectAllowed = "copy"; };
  const dropProject = (event: DragEvent<HTMLElement>) => { event.preventDefault(); const id = event.dataTransfer.getData("application/x-mobius-workspace"); if (id && workspaces.some((item) => item.workspace.id === id)) pin(id); };
  const create = async () => {
    if (!path.trim()) return;
  ...
  return <div className="workspace-atlas">
    <aside className="atlas-project-tree" aria-label={text.tree}>
      ...
      <div className="atlas-tree-scroll">{filtered.map((item) => {
        ...
        return <section key={item.workspace.id} className={`atlas-tree-item ${isSelected ? "selected" : ""}`}>
          <div className="atlas-tree-row" draggable onDragStart={(event) => beginDrag(event, item.workspace.id)}>
    ...
    <section className="atlas-workarea">
      ...
      <section className="recent-workspace-grid" onDragOver={(event) => event.preventDefault()} onDrop={dropProject} aria-label={text.recent}>
