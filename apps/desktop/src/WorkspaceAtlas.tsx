...
function formatSessionTime(value: string, locale: Locale): string {
  const time = new Date(value);
  return Number.isNaN(time.getTime()) ? (locale === "zh-CN" ? "时间未知" : "Unknown time")
    : time.toLocaleString(locale === "zh-CN" ? "zh-CN" : "en", { year: "numeric", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" });
}
...
export function WorkspaceAtlas({ workspaces, reload, openTerminal, onError, onToast, onOpenSessions, locale }: {
  ...
  useEffect(() => {
    ...
    setExpanded((current) => current.has(selectedId) ? current : new Set(current).add(selectedId));
  }, [selectedId]);
      ...
      <div ref={treeScrollRef} className="atlas-tree-scroll" onScroll={(event) => localStorage.setItem("mobius.workspace.tree.scroll-top", String(event.currentTarget.scrollTop))}>{filtered.map((item) => {
        ...
        return <section key={item.workspace.id} className={`atlas-tree-item ${isSelected ? "selected" : ""}`}>
          <div className={`atlas-tree-row ${draggingId === item.workspace.id ? "dragging" : ""}`} data-workspace-id={item.workspace.id} draggable onContextMenu={(event) => { event.preventDefault(); setWorkspaceMenu({ x: event.clientX, y: event.clientY, workspaceId: item.workspace.id }); }} onDragStart={(event) => beginDrag(event, item.workspace.id)} onDragEnd={endDrag} aria-grabbed={draggingId === item.workspace.id} title={locale === "zh-CN" ? "拖动此项目到右侧近期工作区" : "Drag this project to Recent workspaces"}>
            <button className="atlas-expand" type="button" draggable={false} onClick={() => setExpanded((current) => { const next = new Set(current); next.has(item.workspace.id) ? next.delete(item.workspace.id) : next.add(item.workspace.id); return next; })} aria-label={isOpen ? (locale === "zh-CN" ? "折叠工程" : "Collapse project") : (locale === "zh-CN" ? "展开工程" : "Expand project")} aria-expanded={isOpen}>{isOpen ? <ChevronDown size={15}/> : <ChevronRight size={15}/>}</button>
...
export function WorkspaceInspector({ item, initialCheckoutId, openTerminal, onError, onOpenSessions, locale }: { item: WorkspaceView; initialCheckoutId: string | null; openTerminal: (terminal: TerminalInfo) => void; onError: (message: string) => void; onOpenSessions: (focus: SessionFocus) => void; locale: Locale }) {
  const text = words(locale);
  ...
  const loadSessions = useCallback(async () => {
    setLoading(true);
    try {
      const scoped = await desktopApi.querySessions({ query: "", workspace_id: item.workspace.id, checkout_id: checkoutId, providers: [], limit: 160, roots_only: true });
      const checkoutScopes = checkoutId ? [checkoutId] : item.checkouts.map((candidate) => candidate.id);
      const fallback = await Promise.all(checkoutScopes.map((id) => desktopApi.querySessions({ query: "", workspace_id: null, checkout_id: id, providers: [], limit: 160, roots_only: true })));
