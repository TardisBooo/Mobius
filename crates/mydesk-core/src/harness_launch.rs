...
pub fn resolve(agent: &AgentKind, executable: &Path) -> Result<LaunchPrefix, String> {
    ...
    let Some(launcher) = quoted_assignment(&shim, "set \"LAUNCHER=", "\"") else {
        return Ok(direct());
    };
    let launcher = PathBuf::from(launcher);
    let script = fs::read_to_string(&launcher)
        .map_err(|error| format!("Cannot read Harness launcher: {error}"))?;
    let root = launcher
        .parent()
        .ok_or("Harness launcher has no directory")?;
    // Recognize this specific wrapper protocol, not arbitrary PowerShell source.
    if !script.contains("GetEnvironmentVariable('DEEPSEEK_API_KEY', 'User')") {
        return Ok(direct());
    }
    ...
    let command = match agent {
        AgentKind::Pi if script.contains("packages\\coding-agent\\dist\\bundle\\cli.js") => {
            let node_root = quoted_assignment(&script, "$nodeRoot = '", "'")
                .ok_or("Unrecognized Pi Node location")?;
            let node = PathBuf::from(node_root).join("node.exe");
            let cli = root.join("packages/coding-agent/dist/bundle/cli.js");
            if !node.is_file() || !cli.is_file() {
                return Err("Pi native runtime or bundle is missing".into());
            }
            format!(
