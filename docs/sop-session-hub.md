...
| Change | Where | Gate |
| --- | --- | --- |
| Adapter / indexer | `crates/mydesk-core` | `cargo test -p mydesk-core --offline` |
...
| MCP tools | `mydesk-mcp` and `mobius-connect mcp serve` | same tool names; no self-grant |
...
1. Core tests.
2. `mydesk-cli`, `mydesk-mcp`, sibling `mobius-connect` tests.
...
6. Keep desktop (`TardisBooo/Mobius`) and CLI/MCP (`TardisBooo/mobius-connect`) READMEs in the same Agent Session Hub voice. Split map: `docs/REPOS.md`.
